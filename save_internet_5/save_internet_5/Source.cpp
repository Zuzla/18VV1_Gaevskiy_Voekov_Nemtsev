...
//написали программу заново, предыдущая работала некоректно)))
...
int main()
{
	...
	ea[0].grfAccessPermissions = GENERIC_READ | GENERIC_EXECUTE; // Права на чтение и исполнение
	ea[0].grfAccessMode = DENY_ACCESS;							 // Запретить 
	ea[0].grfInheritance = NO_INHERITANCE;
	ea[0].Trustee.TrusteeForm = TRUSTEE_IS_SID;
	ea[0].Trustee.TrusteeType = TRUSTEE_IS_WELL_KNOWN_GROUP;	 // Для группы
	ea[0].Trustee.ptstrName = (LPCH)pEveryoneSID;

	//получение идентификатора группы Администраторы
	SID_IDENTIFIER_AUTHORITY SIDAuthAdmin = SECURITY_NT_AUTHORITY;
	PSID pAdminSID = NULL;

	if (!AllocateAndInitializeSid(&SIDAuthAdmin, 2,
	...
	// Заполнение структуры EXPLICIT_ACCESS для группы Администраторы
	ea[1].grfAccessPermissions = GENERIC_WRITE;	// Права на запись
	ea[1].grfAccessMode = SET_ACCESS;			// Разрешить 
	ea[1].grfInheritance = NO_INHERITANCE;
	ea[1].Trustee.TrusteeForm = TRUSTEE_IS_SID;
	ea[1].Trustee.TrusteeType = TRUSTEE_IS_WELL_KNOWN_GROUP; // Для группы
	ea[1].Trustee.ptstrName = (LPCH)pAdminSID;
	...
	HANDLE hFile = CreateFile("test.txt", GENERIC_ALL, NULL, &sa, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);

	if (hFile) {
		printf("Done! Check test.txt! ");
	}
	else {
